...
class TestBlockChainMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.__configValid = os.path.join(
            self.__currentDir, 'config/valid', 'config.yml')
        self.__configInvalidCorruptHash = os.path.join(
            self.__currentDir, 'config/invalid', 'config_corrupt_hash.yml')
        self.__configInvalidCorruptPreviousHash = os.path.join(
            self.__currentDir, 'config/invalid', 'config_corrupt_previous_hash.yml')
        self.__configInvalidCorruptTransaction = os.path.join(
            self.__currentDir, 'config/invalid', 'config_corrupt_transactions.yml')
    ...
    def test_compute_statement_for_user(self):
        with BlockChain(self.__configValid, self.__currentDir) as bc:
            totalCredit, totalDebit = bc.compute_statement_for_user(
                'userA', False)
