...
class BlockChain:
    ...
    def __applyConfig(self):
        ...
        self.__previousHashForGenesis = self.__ymlConfig.get(
        ...
        if self.__testMode is False:
            ...
            self.__dumpFrequency = self.__ymlConfig.get(
                'dump').get('frequency', 'block')
            if filename is None:
                filename = self.__name
            filename += '.json'
            # form file dump path
            self.__filePath = os.path.join(
                self.__fileRootDir, dirname, filename)
    ...
    def __create_genesis_block(self):
        ...
        if len(self.__chain) == 0:
            ...
            raise Exception(
                'Genesis block can only be created when chain is empty')
    ...
    def add_block(self, listOfTransactions, minerName):
        ...
        if isValid is False:
            raise Exception(self.__name + ' block chain is compromised!',
                            isValid, compromisedBlock.summary)
        ...
        listOfTransactions = LedgerEntry(
            self.__name, minerName, self.__minerReward).transactions + listOfTransactions
        newBlock = Block(None, listOfTransactions,
                         lastBlockInChain.hash, self.__difficulty)
