...
class Block:
    def __init__(self, jsonData, listOfTransaction=[], previousHash=-1, difficulty=0):
        ...
        self.__strData = self.__version + \
            str(self.__previousHash) + str(self.__merkelRoot) + \
            str(self.__timeInSeconds)
    ...
    def initialize(self, listOfTransaction, previousHash, difficulty):
        ...
        self.__hash = -1
        self.__timeInSeconds = int(time.mktime(
        ...
        self.__merkelRoot = self.__build_merkel_root(
            listOfTransactionInStringArray)
    ...
    def __load_from_json(self, jsonData):
        ...
        for t in jsonData.get('transactions'):
            tran = Transaction(t.get('fromUser'), t.get(
                'toUser'), t.get('amount'))
    ...
    def mine(self):
        print('\n\nMining new block chain with difficulty: ' +
